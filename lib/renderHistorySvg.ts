...
function renderHistorySvg(options: RenderHistoryOptions) {
  ...
  const SWIMLANE_CURVE_RADIUS = options?.size?.SWIMLANE_CURVE_RADIUS || 5
  const CIRCLE_RADIUS = options?.size?.CIRCLE_RADIUS || 4
  const CIRCLE_STROKE_WIDTH = options?.size?.CIRCLE_STROKE_WIDTH || 2
